...
namespace MazeGame
{
    class Map
    {
        ...
        // played after basic map elements are set in to change them in the TileMap 2D array 
            ...
            for (int row = 0; row < tileMap.GetLength(0); row++)
            {
                for (int column = 0; column < tileMap.GetLength(1); column++)
                {
                    ...
                    if (player.pRect.Intersects(tileMap[row, column].TileRect))
                    {
                        ...
                        if(tileMap[row,column].Tiletype == TileType.Door && tileMap[row,column].TileColor != FLOORCOLOR)
                        {
                            player.pRect.X = (int)player.previousPos.X;
                            player.pRect.Y = (int)player.previousPos.Y;
                        }
                        if (tileMap[row,column].Tiletype == TileType.AdjacentDoorTiles || tileMap[row, column].Tiletype == TileType.Door)
                        {
                            // Upper tile is door
        ...
        {
            Tile potentialDoor = tileMap[row, column];
            if (potentialDoor.TileTimer > 1 && potentialDoor.TileTimer < 240)
            {
                potentialDoor.TileTimer -= 1;
            }
            else if (potentialDoor.TileTimer == 1)
            {
                potentialDoor.TileTimer = 240;
            }
            if (potentialDoor.TileTimer == 120) // move door to null tile
            {
                potentialDoor.TileColor = Color.Yellow;
            }
        }
        ...
        public void MoveDoors(int row, int column, int doorTimer)
        {
            ...
            {                
                ...
                tileMap[row, column].TileColor = FLOORCOLOR;
            }
